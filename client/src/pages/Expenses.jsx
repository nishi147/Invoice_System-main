      ...
      {showModal && createPortal(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-3 sm:p-4 md:p-6 overflow-y-auto animate-fade-in">
          <div className="relative w-full max-w-lg sm:max-w-xl md:max-w-2xl max-h-[90vh] flex flex-col rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900 overflow-hidden my-auto">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-5 sm:px-6 py-4 dark:border-slate-800 bg-white dark:bg-slate-900 shrink-0">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Enter expense details and optional receipt documentation.
                </p>
              </div>
              <button
            ...
            <form id="expense-form" onSubmit={handleSaveExpense} className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-4 text-xs sm:text-sm">
              <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                ...
                <div>
                  ...
                  {expenseCategory === CREATE_NEW_SENTINEL && (
                    <input
                      ...
                      className="mt-2 w-full rounded-xl border border-brand-400 bg-slate-50/50 py-2.5 px-3.5 outline-none focus:border-brand-500 dark:border-brand-600 text-xs text-slate-900 dark:text-slate-100"
                    />
                  )}
                </div>
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1.5">
                    Expense Amount ({currencySymbol}) <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1.5">Payment Method</label>
                  <select
                    value={paymentMethod}
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1.5">Vendor / Payee</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1.5">Receipt Attachment (PDF/Image)</label>
                  <input
                    type="file"
                    accept="image/*,application/pdf"
                    onChange={(e) => setReceiptFile(e.target.files[0])}
                    className="w-full text-xs text-slate-550 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 dark:file:bg-slate-800 dark:file:text-slate-300 cursor-pointer"
                  />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-slate-700 dark:text-slate-300 font-semibold mb-1.5">Expense Notes</label>
                  <input
                    type="text"
